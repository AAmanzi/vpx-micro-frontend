    ...
    <section className={classNames(style.container, style[color])}>
      <button type='button' className={style.header} onClick={onToggle}>
        ...
        <div className={style.titleWrapper}>
          <h4 className={classNames('primary-text-color', 'body-sm-semibold')}>
            {title}
          </h4>
          <p className='secondary-text-color body-xs-regular'>{description}</p>
        </div>
        <div className={style.countAndChevronWrapper}>
          <div className={classNames('body-sm-bold', style.count)}>{count}</div>
          <Icon
            icon='chevron-right'
